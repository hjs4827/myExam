...
public class HangulCharsetDetector
{
	 ...
	 */
	@NotNull
	public static String toString(@NotNull byte[] ab)
	{
	 ...
	 */
	@NotNull
	public static String toString(@NotNull byte[] ab, int iBegin, int iEnd)
	{
	 ...
	 * @param sHtmlEscapedByteString Html Escape 처리된 Byte String
	 * @return 한글 문자열
	 */
	@NotNull
	public static String toStringAfterUnescape(@NotNull String sHtmlEscapedByteString)
	{
	 ...
	 * @param sByteString byte 배열을 ISO_8859_1 문자셋 사용해서 만든 문자열
	 * @return 한글 문자열
	 */
	@NotNull
	public static String toString(@NotNull String sByteString)
	{
		return toString(sByteString, 0, sByteString.length());
	} // toString()

	/** 바이트 문자열을 한글 문자열로 변환, 바이트 문자열이 아니라면, {@code sByteString}.subsring({@code iBegin}, {@code iEnd}) 값을 리턴함.
	 * @param sByteString
	 * @param iBegin
	 * @param iEnd
	 * @return 한글 문자열
	 */
	@NotNull
	public static String toString(@NotNull String sByteString, int iBegin, int iEnd)
	{
	...
	private static boolean _isDisplayable(@NotNull String s)
	{
		int i = 0;
		int n = s.length();
	 ...
	 */
	public static Charset analyzeCharset(@NotNull byte[] ab) {
	 ...
	 * @return "ASCII" 또는 "UTF-8" 또는 "EUC-KR" 값이 리턴된다.
	 */
	public static Charset analyzeCharset(@NotNull byte[] ab, int iBegin, int iEnd) {
		return _analyzeCharset(ab, iBegin, iEnd, true);
	}

	/** [내부 메소드] 지정한 byte 배열을 분석해서 {"US-ASCII", "UTF-8", "EUC-KR"} 문자셋 중 적합한 하나를 선택한다.
	 * @param ab 바이트 배열
	 * @param iBegin 시작 인덱스
	 * @param iEnd 종료 인덱스
	 * @param bForceToDecide 모호한 경우, 가능성이 높은 Charset을 여기서 결정도록 함.
	 * @return null 은 UTF-8 인지 EUC-KR인지 모호한 경우, 그 외는 "ASCII" 또는 "UTF-8" 또는 "EUC-KR" 값이 리턴된다.
	 */
	private static Charset _analyzeCharset(@NotNull byte[] ab, int iBegin, int iEnd, boolean bForceToDecide)
	{
		int iCharCount = 0;
		int iPos = iBegin;
		while (iPos < iEnd) {
	 ...
	 * @param iBegin
	 * @param iEnd
	 * @return 문자 갯수, 해당 byte 배열이 utf8 only면 양수값, utf8 형식이 아니면 0, uft8 형식에는 맞으나, euc-kr형식일 수도 있으면 음수값.
	 */
	private static int _isUtf8(@NotNull byte[] ab, int iBegin, int iEnd)
	{
		int iCharCount = 0;
	...
	private static final class Utf8BitPatternSequence
	{
		...
		/** N 바이트에 대한 bit 패턴 유형<p>
		 * UTF8 문자는 바이트 크기가 다양하다.
		 * 바이트 크기에 따라 다른 bit 패턴 유형을 나타낸다.
		 */
		private static class BitPatternSequence
		{
			...
			public int isMatch(byte[] ab, int iBegin) {
				int i = 0;
				while (i < BIT_PATTERNS.length && (iBegin + i) < ab.length && BIT_PATTERNS[i].isMatch(ab[iBegin + i])) ++i;
	...
	public static void main(String...args) throws Exception
	{
		final String[] asWord = {
			"유니코드",
			new String("★새해맞이! 1+1+1증정! 4Nº 고농축 앰플 20ml (비타C,아줄렌,히알루론,EGF) 택 1   ".getBytes(CS_UTF_8), CS_ISO_8859_1), // 특수문자 테스트
			new String("★새해맞이! 1+1+1증정! 4Nº 고농축 앰플 20ml (비타C,아줄렌,히알루론,EGF) 택 1".getBytes(CS_EUC_KR), CS_ISO_8859_1),
			new String("º".getBytes(CS_UTF_8), CS_ISO_8859_1),  // 특수문자 테스트
			new String("º".getBytes(CS_EUC_KR), CS_ISO_8859_1),
			"ìì´ë°ì­ì¹¸",
			HtmlUtils.htmlEscape(new String("한".getBytes(CS_UTF_8), CS_ISO_8859_1)),
			HtmlUtils.htmlEscape(new String("한".getBytes(CS_EUC_KR), CS_ISO_8859_1)),
		};
